...
function Payments() {
  ...
  const { retrievePayments, insertPayment, updatePayment, deletePayment } =
    usePayments();
  ...
  const handleUpdateClick = async (e) => {
    e.preventDefault();
    // console.log(option);
    const response = await updatePayment(option);
  ...
  const handleAddClick = async (e) => {
    ...
    const response = await insertPayment(option);
    console.log(response);
    alert(response);
    handleClearClick();
    setReload((count) => {
      ...
      <div className="md:ml-[20rem] p-8">
        ...
        >
          <div className="grid grid-cols-2 gap-4">
            {Object.keys(option)
              .filter((key) => key !== "payment_id" && key !== "status")
              .map((key) => {
              ...
              {payments &&
                payments.map((item) => (
                  ...
                  >
                    ...
                    <Table.Cell>{item.acc_num}</Table.Cell>
                    <Table.Cell>{item.card_name}</Table.Cell>
                    <Table.Cell>{item.contact_num}</Table.Cell>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            const response = confirm(
                              "Are you user you want to delete this payment option?"
                            );
                            if (response) {
                              handleDeleteClick(item.payment_id);
                            }
                          }}
                        >
                          Delete
