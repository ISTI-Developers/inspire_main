...
function Partners() {
  ...
  const { insertPartner, retrievePartners, updatePartner, deletePartner } =
    usePartner();
  ...
  const handleAddClick = async () => {
    ...
    const insert = await insertPartner(name, file);
    alert(insert);
    handleClearClick();
    setReload((count) => {
      return count + 1;
  ...
  const handleUpdateClick = async () => {
    const response = await updatePartner(partner.partners_id, name, file);
    alert(response);
    handleClearClick();
    setPartner(null);
    setReload((count) => {
  ...
  useEffect(() => {
    const setup = async () => {
      const data = await retrievePartners();
      setInformation(data);
    };
    setup();
  }, [reload]);
                        ...
                        <Button
                          ...
                          onClick={() => {
                            const response = confirm(
                              "Are you user you want to delete this partner?"
                            );
                            if (response) {
                              handleDeletePartner(item.partners_id);
                            }
                          }}
                        >
                          Delete
